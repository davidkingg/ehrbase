...
public class FolderAccess extends DataAccess implements I_FolderAccess, Comparable<FolderAccess> {
   ...
   */
  @Override
  public boolean update(final LocalDateTime transactionTime, UUID systemId, UUID committerId, String description,
    ...
    return this.internalUpdate(Timestamp.valueOf(transactionTime), true, null, oldContribution, newContribution,
        systemId, committerId, description, changeType);
  }
  ...
  private Boolean internalUpdate(final Timestamp transactionTime, boolean rootFolder, UUID parentFolder,
      UUID oldContribution, UUID newContribution, UUID systemId, UUID committerId, String description,
    ...
    boolean anySubfolderModified = this.getSubfoldersList() // Map of sub folders with UUID
        ...
        ((FolderAccess) subfolder).internalUpdate(transactionTime, false, updatedFolderId, oldContribution,
            newContribution, systemId, committerId, description, contributionChangeType)))
        .reduce((b1, b2) -> b1 || b2).orElse(false);

    return result || anySubfolderModified;
  }

  private void saveFolderItems(final UUID folderId, final UUID oldContribution, final UUID newContribution,
      final Timestamp transactionTime, DSLContext context, String tenantIdentifier) {

    for (ObjectRef<?> or : this.getItems()) {

      // insert in object_ref
      ObjectRefRecord orr = new ObjectRefRecord(or.getNamespace(), or.getType(), UUID.fromString(or.getId().getValue()),
          newContribution, transactionTime, folderRecord.getSysPeriod(), tenantIdentifier);
      context.attach(orr);
      orr.store();

      // insert in folder_item
      FolderItemsRecord fir = new FolderItemsRecord(folderId, UUID.fromString(or.getId().getValue()), newContribution,
          transactionTime, folderRecord.getSysPeriod(), tenantIdentifier);
      context.attach(fir);
      fir.store();
    }
  }
  ...
  public UUID commit(LocalDateTime transactionTime, UUID systemId, UUID committerId, String description) {
    ...
    this.contributionAccess.commit(Timestamp.valueOf(transactionTime), committerId, systemId,
        ContributionDataType.folder, ContributionDef.ContributionState.COMPLETE,
        I_ConceptAccess.ContributionChangeType.CREATION, description);
  ...
  public UUID commit(LocalDateTime transactionTime, UUID contributionId) {
    ...
    this.saveFolderItems(this.getFolderRecord().getId(), contributionId, contributionId,
        Timestamp.valueOf(transactionTime), getContext(), getFolderRecord().getNamespace());
    ...
    this.getSubfoldersList().values().forEach(child -> {
      ...
      FolderHierarchyRecord fhRecord = this.buildFolderHierarchyRecord(this.getFolderRecord().getId(),
          ((FolderAccess) child).getFolderRecord().getId(), contributionId, Timestamp.valueOf(transactionTime));
      fhRecord.store();
    });
  ...
  public static I_FolderAccess retrieveInstanceForExistingFolder(I_DomainAccess domainAccess, UUID folderId) {
    ...
    Table<?> initialTable = table(select().from(FOLDER_HIERARCHY).where(FOLDER_HIERARCHY.PARENT_FOLDER.eq(folderId)));

    Field<UUID> subfolderChildFolder = field("subfolders.{0}", FOLDER_HIERARCHY.CHILD_FOLDER.getDataType(),
        FOLDER_HIERARCHY.CHILD_FOLDER.getUnqualifiedName());
    Result<Record> folderSelectedRecordSub = domainAccess.getContext().withRecursive(SUBFOLDERS)
        .as(select(ArrayUtils.addAll(initialTable.fields(), folderTable.fields())).from(initialTable)
            .leftJoin(folderTable)
            .on(initialTable.field(PARENT_FOLDER, FOLDER_HIERARCHY.PARENT_FOLDER.getType())
                .eq(folderTable.field("id", FOLDER.ID.getType())))
            .union(
                (select(ArrayUtils.addAll(sfTable.fields(), folderTable2.fields())).from(sfTable).innerJoin(SUBFOLDERS)
                    .on(sfTable.field(PARENT_FOLDER, FOLDER_HIERARCHY.PARENT_FOLDER.getType())
                        .eq(subfolderChildFolder))).leftJoin(folderTable2)
                            .on(folderTable2.field("id", FOLDER.ID.getType()).eq(subfolderChildFolder))))
    ...
    for (Record rec : folderSelectedRecordSub) {
      ...
      fHierarchyMap.get(rec.getValue(PARENT_FOLDER, UUID.class)).put((UUID) rec.getValue(CHILD_FOLDER),
          buildFolderAccessFromFolderId((UUID) rec.getValue(CHILD_FOLDER), domainAccess, folderSelectedRecordSub));
    }

    // 3 - populate result and return
    return FolderAccess.buildFolderAccessHierarchy(fHierarchyMap, folderId, null, folderSelectedRecordSub,
        domainAccess);
  }
   ...
   */
  public static Set<ObjectVersionId> retrieveFolderVersionIdsInContribution(I_DomainAccess domainAccess,
      UUID contribution, String nodeName) {
    ...
    domainAccess.getContext().select(FOLDER_HISTORY.ID).from(FOLDER_HISTORY)
        .where(FOLDER_HISTORY.IN_CONTRIBUTION.eq(contribution)).fetch().forEach(rec -> folders.add(rec.value1()));
  ...
  private static Map<Record, Integer> getVersionMapOfFolder(I_DomainAccess domainAccess, UUID folderId) {
    ...
    Result<FolderHistoryRecord> historyRecords = domainAccess.getContext().selectFrom(FOLDER_HISTORY)
        .where(FOLDER_HISTORY.ID.eq(folderId)).orderBy(FOLDER_HISTORY.SYS_TRANSACTION.desc()).fetch();
   ...
   */
  public static I_FolderAccess getNewFolderAccessInstance(final I_DomainAccess domainAccess, final Folder folder,
      final DateTime dateTime, final UUID ehrId, String tenantIdentifier) {
  ...
  private Integer delete(final UUID folderId, UUID contribution, UUID systemId, UUID committerId, String description) {
    ...
    var delAudit = I_AuditDetailsAccess.getInstance(this, systemId, committerId,
        I_ConceptAccess.ContributionChangeType.DELETED, description, this.folderRecord.getNamespace());
   ...
   */
  private static I_FolderAccess buildFolderAccessHierarchy(final Map<UUID, Map<UUID, I_FolderAccess>> fHierarchyMap,
      final UUID currentFolder, final I_FolderAccess parentFa, final Result<Record> folderSelectedRecordSub,
   ...
   */
  private static FolderAccess buildFolderAccessFromGenericRecord(final Record folderRecord,
      final I_DomainAccess domainAccess) {
    ...
    folderAccess
        .setFolderNArchetypeNodeId(folderRecord.get(org.ehrbase.jooq.pg.tables.Folder.FOLDER.ARCHETYPE_NODE_ID));
    folderAccess.setIsFolderActive(folderRecord.get(org.ehrbase.jooq.pg.tables.Folder.FOLDER.ACTIVE));
    folderAccess.setFolderDetails(folderRecord.get(org.ehrbase.jooq.pg.tables.Folder.FOLDER.DETAILS.getName(),
        new OtherDetailsJsonbBinder().converter()));
    folderAccess.setFolderSysTransaction(folderRecord.get(org.ehrbase.jooq.pg.tables.Folder.FOLDER.SYS_TRANSACTION));
    folderAccess.setFolderSysPeriod(folderRecord.get(org.ehrbase.jooq.pg.tables.Folder.FOLDER.SYS_PERIOD.getName(),
        new SysPeriodBinder().converter()));
  ...
  private static FolderAccess buildFolderAccessFromFolderRecord(final FolderRecord folderRecord,
    ...
    folderAccess.getItems().addAll(FolderAccess.retrieveItemsByFolderAndContributionId(folderRecord.getId(),
        folderRecord.getInContribution(), domainAccess));
    return folderAccess;
  }
   ...
   */
  private static FolderAccess buildFolderAccessFromFolderId(final UUID id, final I_DomainAccess domainAccess,
   ...
   *         {@link com.nedap.archie.rm.directory.Folder}
   */
  public static I_FolderAccess buildPlainFolderAccess(final I_DomainAccess domainAccess, final Folder folder,
      final Timestamp timestamp, final UUID ehrId, final I_ContributionAccess contributionAccess, String tenantIdentifier) {
   ...
   * @return
   */
  private static List<ObjectRef<?>> retrieveItemsByFolderAndContributionId(UUID folderId, UUID inContribution,
      I_DomainAccess domainAccess) {
    Result<Record> retrievedRecords = domainAccess.getContext()
        .with("folderItemsSelect")
          .as(select(FOLDER_ITEMS.OBJECT_REF_ID.as("object_ref_id"), FOLDER_ITEMS.IN_CONTRIBUTION.as("item_in_contribution")).from(FOLDER_ITEMS).where(FOLDER_ITEMS.FOLDER_ID.eq(folderId)))
        .select()
          .from(OBJECT_REF, table(name("folderItemsSelect")))
          .where(
              field(name("object_ref_id"), FOLDER_ITEMS.OBJECT_REF_ID.getType())
                .eq(OBJECT_REF.ID)
                .and(field(name("item_in_contribution"), FOLDER_ITEMS.IN_CONTRIBUTION.getType()).eq(OBJECT_REF.IN_CONTRIBUTION))
          )
        .fetch();

    List<ObjectRef<?>> result = new ArrayList<>();
    for (Record recordRecord : retrievedRecords) {
      Record9<String, String, UUID, UUID, Timestamp, AbstractMap.SimpleEntry<OffsetDateTime, OffsetDateTime>, String, UUID, UUID> recordParam = (Record9<String, String, UUID, UUID, Timestamp, AbstractMap.SimpleEntry<OffsetDateTime, OffsetDateTime>, String, UUID, UUID>) recordRecord;
   ...
   */
  private static I_FolderAccess buildFolderAccessTreeRecursively(final I_DomainAccess domainAccess,
      final Folder current, final FolderAccess parent, final DateTime dateTime, final UUID ehrId,
      final I_ContributionAccess contributionAccess, String tenantIdentifier) {
    ...
    folderAccess = FolderAccess.buildPlainFolderAccess(domainAccess, current, Timestamp.from(Instant.now()), ehrId,
        contributionAccess, tenantIdentifier);
    ...
    for (Folder child : current.getFolders()) {
      buildFolderAccessTreeRecursively(domainAccess, child, (FolderAccess) folderAccess, dateTime, ehrId,
          ((FolderAccess) folderAccess).getContributionAccess(), tenantIdentifier);
    }
    return folderAccess;
  }
   ...
   */
  public static I_FolderAccess buildNewFolderAccessHierarchy(final I_DomainAccess domainAccess, final Folder folder,
      final Timestamp timeStamp, final UUID ehrId, final I_ContributionAccess contributionAccess, String tenantIdentifier) {
    // Create access for the current folder
    I_FolderAccess folderAccess = buildPlainFolderAccess(domainAccess, folder, timeStamp, ehrId, contributionAccess, tenantIdentifier);
    ...
    if (folder.getFolders() != null && !folder.getFolders().isEmpty()) {
      ...
      folder.getFolders().forEach(child -> {
        ...
        I_FolderAccess childFolderAccess = buildNewFolderAccessHierarchy(domainAccess, child, timeStamp, ehrId, contributionAccess, tenantIdentifier);
   ...
   */
  private FolderHierarchyRecord buildFolderHierarchyRecord(final UUID parentFolder, final UUID childFolder,
      final UUID inContribution, final Timestamp sysTransaction) {
   ...
   */
  public static int getVersionNumberAtTime(I_DomainAccess domainAccess, final ObjectVersionId rootFolderId,
    ...
    int folderCount = domainAccess.getContext().fetchCount(FOLDER,
        FOLDER.ID.equal(folderUuid).and(FOLDER.SYS_TRANSACTION.lessOrEqual(sysTransaction)));

    // Count all history entries for the root folder
    int folderHistoryCount = domainAccess.getContext().fetchCount(FOLDER_HISTORY,
        FOLDER_HISTORY.ID.equal(folderUuid).and(FOLDER_HISTORY.SYS_TRANSACTION.lessOrEqual(sysTransaction)));
    ...
    if (folderHistoryCount <= 0) {
      ...
      if (folderCount <= 0) {
        ...
        throw new ObjectNotFoundException("directory",
  ...
  public static Timestamp getTimestampForVersion(I_DomainAccess domainAccess, final ObjectVersionId rootFolderId,
      Integer version) {
    ...
    if (currentVersion > version) {
      ...
      Result<FolderHistoryRecord> folderHistoryRecords = domainAccess.getContext().selectFrom(FOLDER_HISTORY)
          .where(FOLDER_HISTORY.ID.equal(rootFolderUuid)).orderBy(FOLDER_HISTORY.SYS_TRANSACTION.desc())
          .limit(currentVersion - version).fetch();
